...
let __loadSeq=0;
async function gj(u){const r=await fetch(u,{cache:"no-store"});if(!r.ok)throw Error(`${u}:${r.status}`);return r.json()}
async function gp(u){const r=await fetch(u,{cache:"no-store"});if(!r.ok)throw Error(`${u}:${r.status}`);const b=await r.arrayBuffer(),v=new DataView(b);if(String.fromCharCode(...new Uint8Array(b,0,4))!=="LTS4")throw Error("bad LTS4");const n=v.getUint32(4,true),xyz=new Float32Array(n*3),lab=new Uint8Array(n),score=new Float32Array(n);let o=8;for(let i=0;i<n;i++,o+=20){xyz[3*i]=v.getFloat32(o,true);xyz[3*i+1]=v.getFloat32(o+4,true);xyz[3*i+2]=v.getFloat32(o+8,true);lab[i]=v.getUint8(o+12);score[i]=v.getFloat32(o+16,true)}return{n,xyz,lab,sc:score}}
...
async function load(i){const seq=++__loadSeq;const idx=Math.max(0,Math.min(S.m.frame_count-1,Number(i)||0));$("#frame").value=idx;const f=await gj(`data/frames/${String(idx).padStart(6,"0")}.json?frame=${idx}&seq=${seq}`);const pts=await gp(`${f.semantic_point_file}?frame=${idx}&seq=${seq}`);if(seq!==__loadSeq)return;S.idx=idx;S.f=f;S.pts=pts;const sf=Number(S.f.source_frame_index??idx);$("#fv").textContent=`F${String(sf).padStart(3,"0")} · ${idx+1}/${S.m.frame_count}`;buildGround();S.lane=null;buildCams();panel();updateStaticCount();draw();document.documentElement.dataset.ltmapFrame=String(sf);document.documentElement.dataset.ltmapPoints=String(S.pts.n)}
